...
public class TimeSlotController {
    ...
    public ResponseEntity<TimeSlotsResponse> getTimeSlots(
            @Parameter(description = "Language code filter")
            @RequestParam(required = false) String language,
            @Parameter(description = "CEFR level filter")
            @RequestParam(required = false) TimeSlot.Level level,
        ...
        TimeSlotsResponse response = timeSlotService.findTimeSlots(language, level, fromDate, toDate, page, size);
    ...
    @DeleteMapping("/{id}")
    @Operation(summary = "Delete time slot", description = "Delete (deactivate) a time slot (admin only)")
