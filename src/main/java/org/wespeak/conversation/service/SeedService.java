...
public class SeedService {
  ...
  public Map<String, Object> seedDatabase() {
    ...
    return Map.of("status", "success", "timeSlotsCreated", slots.size());
  }
  ...
  private List<TimeSlot> createSlotsForLanguage(String language, Instant baseTime) {
    List<TimeSlot> slots = new ArrayList<>();
    TimeSlot.Level[] levels = {
      TimeSlot.Level.A1, TimeSlot.Level.A2, TimeSlot.Level.B1, TimeSlot.Level.B2
    };
    ...
    for (int day = 0; day < 7; day++) {
      ...
      for (int hour = 9; hour <= 11; hour++) {
        Instant slotTime =
            dayStart.plus(
                hour - baseTime.atZone(java.time.ZoneOffset.UTC).getHour(), ChronoUnit.HOURS);
