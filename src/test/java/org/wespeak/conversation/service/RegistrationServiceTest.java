...
class RegistrationServiceTest {
  ...
  void register_shouldCreateRegistration() {
    ...
    when(registrationRepository.existsByTimeSlotIdAndUserIdAndStatus(any(), any(), any()))
        .thenReturn(false);
  ...
  void register_shouldThrowWhenSlotNotFound() {
    ...
    // When/Then
    assertThrows(
        ResourceNotFoundException.class, () -> registrationService.register("invalid", userId));
  }
  ...
  void register_shouldThrowWhenAlreadyRegistered() {
    ...
    when(registrationRepository.existsByTimeSlotIdAndUserIdAndStatus(
            "slot-1", userId, Registration.Status.registered))
        .thenReturn(true);

    // When/Then
    RegistrationException ex =
        assertThrows(
            RegistrationException.class, () -> registrationService.register("slot-1", userId));
  ...
  void register_shouldThrowWhenMaxRegistrationsReached() {
    ...
    when(registrationRepository.existsByTimeSlotIdAndUserIdAndStatus(any(), any(), any()))
        .thenReturn(false);
    when(registrationRepository.countByUserIdAndStatus(userId, Registration.Status.registered))
        .thenReturn(3L);
  ...
  void unregister_shouldCancelRegistration() {
    ...
    verify(registrationRepository)
        .save(argThat(reg -> reg.getStatus() == Registration.Status.cancelled));
  }
